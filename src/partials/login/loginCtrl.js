angular.module('acme').controller('loginCtrl', function ($scope, $rootScope, apiService, ipCookie, $window, $location, userService, $translate) {
	...
	$scope.newuser = function () {
		...
		newpublicKey = AcmeJS.crypto.getKeys($scope.newsecret).publicKey;
	...
	$scope.saveTxt = function (filename) {
		...
		var address = AcmeJS.crypto.getAddress(newpublicKey);
		txt = 'secret:' + '\r\n' + text + '\r\n\r\n' + 'address:' + '\r\n' + address + '\r\n';
	...
	$scope.registerin = function () {
		...
		var publicKey = AcmeJS.crypto.getKeys($scope.secret).publicKey;
