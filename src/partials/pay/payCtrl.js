angular.module('acme').controller('payCtrl', function ($scope, $rootScope, $filter, apiService, ipCookie, $http, $window, userService, postSerivice, $translate) {
    ...
    $scope.calculateFee = function () {
        if ($scope.amount && Number($scope.amount) > 0) {
            ...
            var fee = AcmeJS.transaction.calculateFee(amount);
    ...
    $scope.sentMsg = function () {
        ...
        var transaction = AcmeJS.transaction.createTransaction(String($scope.fromto), amount, userService.secret, $scope.secondPassword);
